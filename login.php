...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        if ($result->num_rows === 0) {
            ...
            // Check lockout timing
            $now = new DateTime();
            $lastFailed = $user['last_failed_login'] ? new DateTime($user['last_failed_login']) : null;
            $minutesSinceLastFail = $lastFailed
                ? $now->diff($lastFailed)->i + ($now->diff($lastFailed)->h * 60) + ($now->diff($lastFailed)->d * 1440)
                : 0;

            if ($user['failed_attempts'] >= 3 && $minutesSinceLastFail < 45) {
                $remaining = 45 - $minutesSinceLastFail;
                $error = "Too many failed attempts. Try again after {$remaining} minute(s).";
